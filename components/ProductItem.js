...
export default function ProductItem({ products, productKey }) {
    const { category, price, sizes, colors, img, imgAlt, slug } =
        products[productKey];
            ...
            <Link href={`/product/${slug}`}>
                <a>
                    <div className="relative h-56 rounded overflow-hidden">
                        <Image
                            src={img}
                            alt={imgAlt || productKey}
                            layout="fill"
                            objectFit="contain"
                        />
                    </div>
                    <div className="mt-4 space-y-1">
                        <h5 className="text-gray-500 text-xs tracking-widest mb-1 capitalize">
                            {category}
                        </h5>
                        ...
                        <p className="mt-1 text-sm space-x-2">
                            {colors.map((color, i) => (
                                <span
                                    key={i}
